    ...
    def forward(self,input_feature_block,emb, x, img, ws, att_vars, fused_modconv = None, **layer_kwargs):
        ...
        if self.stem:
            ...
            x, att_maps[0], att_vars = self.conv0(None, x, next(w_iter), att_vars,
                                                  fused_modconv=fused_modconv,
                                                  feature_from_encoder=input_feature_block[-4],
                                                  **layer_kwargs)
            ...
            # x = y.add_(x)
            x = self.spade0(x,feature_from_encoder=input_feature_block[-2])
        ...
        if self.is_last or self.architecture == "skip":
            y = self.torgb(input_feature_block[-1],x, next(w_iter), fused_modconv = fused_modconv)
...
class BipartiteDecoder_1spade_catFeat_skip(torch.nn.Module):
    def __init__(self,
