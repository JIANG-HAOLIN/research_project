...
class DecoderBlock_1spade(torch.nn.Module):
    ...
    def forward(self,input_feature_block,emb, x, img, ws, att_vars, fused_modconv = None, **layer_kwargs):
        ...
        if self.stem:
            ...
            init_noise = torch.randn(batch_size,self.in_channels//2,self.in_res,2*self.in_res,device=ws.device)
            init_emb = emb[f'res{self.in_res}'][0]
            x = torch.cat([init_emb,init_noise],dim = 1)
            x = convert(x)
            att_maps = [None, None]
            output_features = []
...
class BipartiteDecoder_1spade_catFeature_3D(torch.nn.Module):
    def __init__(self,
