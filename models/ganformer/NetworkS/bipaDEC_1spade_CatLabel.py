...
class TransformerLayer(torch.nn.Module):
    def __init__(self,
        ...
        self.to_queries = FullyConnectedLayer(self.from_dim + 35 if self.kmeans else self.from_dim,dim)
...
class DecodingLayer(torch.nn.Module):
    ...
    def forward(self,input_feature_layer, x, y, att_vars = None, pos = None, mask = None,
        ...
        if self.transformer is not None:
            label = torch.nn.functional.interpolate(_kwargs['label'],size=(self.out_res, 2*self.out_res),mode="nearest")
...
class BipartiteDecoder_1spade_catLabel(torch.nn.Module):
    def __init__(self,
