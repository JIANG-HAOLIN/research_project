...
class SPADE(torch.nn.Module):
    def __init__(self, config_text, norm_nc, label_nc):
        ...
        if param_free_norm_type == 'instance':
            self.param_free_norm = torch.nn.InstanceNorm2d(norm_nc, affine=False)
        ...
        self.mlp_shared = torch.nn.Sequential(
            torch.nn.Conv2d(label_nc, nhidden, kernel_size=ks, padding=pw),
            torch.nn.ReLU()
        )
...
class DecodingLayer(torch.nn.Module):
    ...
    def forward(self,input_feature_layer, x, y, att_vars = None, pos = None, mask = None, noise_mode = "random", fused_modconv = True,label=None,**_kwargs):
...
class BipartiteDecoder_cat(torch.nn.Module):
    def __init__(self,
