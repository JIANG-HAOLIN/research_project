...
class TransformerLayer(torch.nn.Module):
    def __init__(self,
        ...
        self.to_queries = FullyConnectedLayer(self.from_dim*2 if self.kmeans == True else self.from_dim,
                                                      dim)
...
class DecoderToRGBLayer(torch.nn.Module):
    def __init__(self, in_channels, out_channels, y_dim, kernel_size = 1, style = True):
        ...
        #
        self.param_free_norm = torch.nn.InstanceNorm2d(out_channels, affine=False)  ##jhl
        self.spade_weight_modulation = EqualLinear(128, out_channels, bias_init=1)  # jhl
        self.spade_bias_modulation = EqualLinear(128, out_channels, bias_init=1)  # jhl
...
class DecoderBlock_1spade(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs                      # Arguments for SynthesisLayer
        ...
        if self.stem:
            ...
            self.project0 = torch.nn.Sequential(torch.nn.Conv2d(67,out_channels,kernel_size=3,padding=1),
                                                    torch.nn.ReLU())
            self.conv1 = DecodingLayer(in_channels, out_channels,
                                       ...
                                       y_dim=w_dim, style=style, **layer_kwargs)
            self.project1 = torch.nn.Sequential(torch.nn.Conv2d(67,out_channels,kernel_size=3,padding=1),
                                                    torch.nn.ReLU())
            self.num_conv += 2
            self.project2 = torch.nn.Sequential(torch.nn.Conv2d(67,128,kernel_size=3,padding=1),
                                                    torch.nn.ReLU())
    ...
    def forward(self,input_feature_block,emb,block_label, x, img, ws, att_vars, fused_modconv = None, **layer_kwargs):
        ...
        if self.stem:
            ...
            x, att_maps[0], att_vars = self.conv0(None, x, next(w_iter), att_vars,
                                                  fused_modconv=fused_modconv,label=self.project0(block_label),
                                                  **layer_kwargs)
            output_features.append(x)
            x = torch.cat([x, emb[f'res{self.out_res}'][0]],dim=1)
            x, att_maps[0], att_vars = self.conv1(None, x, next(w_iter), att_vars,
                                                  fused_modconv=fused_modconv,label=self.project1(block_label),
                                                  **layer_kwargs)
        ...
        if self.is_last or self.architecture == "skip":
            ...
            y = self.torgb(self.project2(block_label),x, next(w_iter), fused_modconv = fused_modconv)
...
class BipDC_catFeat_skipSPD_3Dnoise_noEC(torch.nn.Module):
    def __init__(self,
