...
class DecoderBlock_1spade(torch.nn.Module):
    ...
    def forward(self,input_feature_block,emb,label, x, img, ws, att_vars, fused_modconv = None, **layer_kwargs):
        ...
        if self.is_last: # For backward compatibility with TF version
            x = self.conv_last(None, x, next(w_iter), att_vars,
                               fused_modconv=fused_modconv, **layer_kwargs)[0]
        if self.is_last or self.architecture == "skip":
            y = self.torgb(input_feature_block[-2],x, next(w_iter), fused_modconv = fused_modconv)
...
class BipartiteDecoder_catlabel_skipSPD_3Dnoise_endspd(torch.nn.Module):
    def __init__(self,
