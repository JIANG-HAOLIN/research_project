...
class TransformerLayer(torch.nn.Module):
    def __init__(self,
        ...
        if is_D:
            self.to_queries = FullyConnectedLayer(35+from_dim,dim)
        # self.to_queries = FullyConnectedLayer(from_dim,dim)
...
class DecoderBlock_1spade(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs                      # Arguments for SynthesisLayer
        ...
        if is_last: # For backward compatibility with TF version
            ...
            self.squeeze_excitation_layer = squeeze_citation()
    ...
    def forward(self,input_feature_block,emb,label, x, img, ws, att_vars, fused_modconv = None, **layer_kwargs):
        ...
        if self.is_last: # For backward compatibility with TF version
            scale = self.squeeze_excitation_layer(x)
            x = scale*x
...
class BipartiteDecoder_catlabel_skipSPD_3Dnoise_simplex_SE(torch.nn.Module):
    def __init__(self,
...
class squeeze_citation(torch.nn.Module):
    def __init__(self,kernel_size=(256,512),in_features=64,r = 2):
        super().__init__()
        self.global_pooling_layer = torch.nn.AvgPool2d(kernel_size=kernel_size)
        self.FC_1 = torch.nn.Conv2d(kernel_size=1,in_channels=in_features,out_channels=int(in_features/r))
        self.act_1 = torch.nn.ReLU()
        self.FC_2 = torch.nn.Conv2d(kernel_size=1,in_channels=int(in_features/r),out_channels=in_features)
