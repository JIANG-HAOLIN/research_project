...
class SPADE_norm_denorm(torch.nn.Module):
    ...
    def __init__(self,in_channels, out_channels, kernel_size=1, bias=None, up=None,resample_kernel=None, gain=None,out_resolution=None):
        super().__init__()
        # pw = kernel_size // 2
        # if out_channels == 3:
        ...
        if self.out_channels == 3:
            self.param_free_norm = torch.nn.LayerNorm([out_channels,out_resolution,2*out_resolution],elementwise_affine=False)  ##jhl
...
class skip_op(torch.nn.Module):
    ...
    def __init__(self,in_channels, out_channels, kernel_size, bias, up,resample_kernel, gain,spade=False, out_resolution = None):
        super().__init__()
        ...
        self.spade = spade
        if self.spade == True:
            self.spade_norm = SPADE_norm_denorm(in_channels=in_channels,out_channels=out_channels,out_resolution=out_resolution)
        ...
        if self.stem:
            ...
            if self.architecture == 'resnet':
                self.skip = skip_op(in_channels, out_channels, kernel_size=1, bias=False, up = 1,
                                    ...
                                    spade=True,out_resolution=self.out_res)
...
class BipartiteDecoder_catlabel_skipSPD_3Dnoise_simplex_layernorm(torch.nn.Module):
    def __init__(self,
