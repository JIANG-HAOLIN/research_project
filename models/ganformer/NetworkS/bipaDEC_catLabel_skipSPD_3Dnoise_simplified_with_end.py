...
class TransformerLayer(torch.nn.Module):
    def __init__(self,
        ...
        self.to_queries = FullyConnectedLayer(self.from_dim + 35 if self.kmeans == True else self.from_dim,
                                                      dim)
...
class SPADE_norm_denorm(torch.nn.Module):
    ...
    def __init__(self,in_channels, out_channels, kernel_size=1, bias=None, up=None,resample_kernel=None, gain=None):
        ...
        # pw = kernel_size // 2
        # if out_channels == 3:
        #     self.param_free_norm = torch.nn.InstanceNorm2d(out_channels, affine=False)
        #     self.spade_weight_modulation = torch.nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=pw)
        #     self.spade_bias_modulation = torch.nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=pw)
        # else:
        #     self.param_free_norm = torch.nn.InstanceNorm2d(out_channels, affine=False)  ##jhl
        #     self.spade_weight_modulation = torch.nn.Conv2d(out_channels, out_channels, kernel_size=kernel_size, padding=pw)
        #     self.spade_bias_modulation = torch.nn.Conv2d(out_channels, out_channels, kernel_size=kernel_size, padding=pw)
        self.in_channels = in_channels
        self.out_channels = out_channels
        if self.out_channels == 3:
            self.param_free_norm = torch.nn.InstanceNorm2d(self.out_channels, affine=False)  ##jhl
            self.spade_weight_modulation = EqualLinear(self.in_channels, self.out_channels, bias_init=1)  # jhl
            self.spade_bias_modulation = EqualLinear(self.in_channels, self.out_channels, bias_init=1)  # jhl
        else:
            self.param_free_norm = torch.nn.InstanceNorm2d(self.out_channels, affine=False)  ##jhl
            self.spade_weight_modulation = EqualLinear(self.out_channels, self.out_channels, bias_init=1)  # jhl
            self.spade_bias_modulation = EqualLinear(self.out_channels, self.out_channels, bias_init=1)  # jhl

    def forward(self,x,feature_from_encoder):
        # x = self.param_free_norm(x)
        # spade_weight = self.spade_weight_modulation(feature_from_encoder)
        # spade_bias = self.spade_bias_modulation(feature_from_encoder)
        # x = x * (1+spade_weight) + spade_bias
        x = self.param_free_norm(x)
...
# class skip_op(torch.nn.Module):
#
#     def __init__(self,in_channels, out_channels, kernel_size, bias, up,resample_kernel, gain,spade=False):
#         super().__init__()
...
#         x = self.skip(x)
#
#         if self.spade == True and feature_from_encoder is not None:
#             x = self.spade_norm(x,feature_from_encoder)
#
#         return x


class skip_op_OASIS(torch.nn.Module):
    ...
    def __init__(self,in_channels, out_channels, kernel_size, bias, up,resample_kernel, gain,spade=False):
        ...
        self.spade = spade
        if self.spade == True:
            self.spade_norm = SPADE_OASIS(norm_nc=out_channels,label_nc=99)
            print('spade layer activated in skip connection')
...
class DecoderToRGBLayer(torch.nn.Module):
    def __init__(self, in_channels, out_channels, y_dim, kernel_size = 1, style = True):
        ...
        #
        # self.param_free_norm = torch.nn.InstanceNorm2d(out_channels, affine=False)  ##jhl
        # self.spade_weight_modulation = EqualLinear(in_channels, out_channels, bias_init=1)  # jhl
        # self.spade_bias_modulation = EqualLinear(in_channels, out_channels, bias_init=1)  # jhl
...
class DecodingLayer(torch.nn.Module):
    ...
    def forward(self,input_feature_layer, x, y, att_vars = None, pos = None, mask = None,
        ...
        label = torch.nn.functional.interpolate(label,size=[self.out_res,2*self.out_res],mode='nearest')
...
@persistence.persistent_class
class DecoderBlock_1spade(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs                      # Arguments for SynthesisLayer
        ...
        if self.stem:
            ...
            if self.architecture == 'resnet':
                self.skip = skip_op_OASIS(in_channels, out_channels, kernel_size=1, bias=False, up = 1,
            ...
            if self.architecture == 'resnet':
                self.skip = skip_op_OASIS(in_channels, out_channels, kernel_size=1, bias=False, up = 2,
                                    resample_kernel=resample_kernel, gain=get_gain(architecture),
                                    spade = True)
        ...
        if is_last: # For backward compatibility with TF version
            ...
            self.conv_last = DecodingLayer(in_channels, out_channels,
                                           ...
                                           **conv_last_kwargs)
            self.spade_last = SPADE_OASIS(norm_nc=out_channels,label_nc=99)
            self.num_conv += 1
    ...
    def forward(self,input_feature_block,emb,label, x, img, ws, att_vars, fused_modconv = None, **layer_kwargs):
        ...
        if self.stem:
            ...
            att_maps = [None, None]
            output_features = []
            y = self.skip(x,feature_from_encoder=input_feature_block)
        ...
        if self.is_last: # For backward compatibility with TF version
            x = self.conv_last(None,x, next(w_iter), fused_modconv = fused_modconv,
                               **layer_kwargs)[0]
            x = self.spade_last(x,input_feature_block)

        if self.is_last or self.architecture == "skip":
            y = self.torgb(None,x, next(w_iter), fused_modconv = fused_modconv)
...
# - StyleGAN (--style)
@persistence.persistent_class
class DecoderNetwork_1spade(torch.nn.Module):
    def __init__(self,
        w_dim,                         # Intermediate latent (W) dimensionality
        k,                             # Number of latent vector components w_1,...,w_k
        ...
        **block_kwargs                 # Arguments for SynthesisBlock
        ...
        for i,res in enumerate(self.block_resolutions):
            ...
            block = DecoderBlock_1spade(in_channels, out_channels, w_dim = w_dim, k = k,
                                 out_resolution = res,
    ...
    def forward(self,from_encoder,emb,label, ws, **block_kwargs):
        ...
        z = torch.randn(label.shape[0], 64 , dtype=torch.float32, device=label.device)
        z = z.view(z.size(0), z.size(1), 1, 1)
        z = z.expand(z.size(0), z.size(1), label.size(2), label.size(3))
        from_encoder = torch.cat((label, z), dim=1)
        ...
        for res, cur_ws in (zip(self.block_resolutions, block_ws)):
            input_feature_block = F.interpolate(from_encoder,size=[res,2*res],mode='nearest')
            block = getattr(self, f"b{res}")
            x, img, _att_maps, att_vars,output_features = block(input_feature_block,emb,label,x, img, cur_ws, att_vars, **block_kwargs)
            att_maps += _att_maps
...
class BipartiteDecoder_catlabel_skipSPD_3Dnoise_OASIS_withend(torch.nn.Module):
    def __init__(self,
        ...
        self.decoder = DecoderNetwork_1spade(w_dim=w_dim, k=k, img_resolution=img_resolution,
...
class SPADE_OASIS(nn.Module):
    def __init__(self, norm_nc, label_nc,opt=None):
        super().__init__()
        self.first_norm = get_norm_layer(norm_nc,opt=None)
        if opt != None:
            ks = opt.spade_ks
        else:ks = 3
        nhidden = 128
...
def get_norm_layer( norm_nc,opt=None):
