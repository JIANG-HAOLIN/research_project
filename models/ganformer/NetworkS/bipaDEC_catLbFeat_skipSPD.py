...
class TransformerLayer(torch.nn.Module):
    def __init__(self,
        ...
        self.to_queries = FullyConnectedLayer(self.from_dim*2+35 if self.kmeans else self.from_dim,dim)
...
class DecodingLayer(torch.nn.Module):
    ...
    def forward(self,input_feature_layer, x, y, att_vars = None, pos = None, mask = None,
        ...
        if self.transformer is not None:
            ...
            x_cat = torch.cat([x, feature_from_encoder, label], dim=1)
            shape = x.shape
...
class DecoderBlock_1spade(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs                      # Arguments for SynthesisLayer
        ...
        print('catLbFeat_skipSPD constructed with right order ')
...
class BipartiteDecoder_catLbFeat_skipSPD(torch.nn.Module):
    def __init__(self,
