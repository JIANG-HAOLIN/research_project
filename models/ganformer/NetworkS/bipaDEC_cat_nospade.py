...
class TransformerLayer(torch.nn.Module):
    def __init__(self,
        ...
        self.to_queries = FullyConnectedLayer(self.from_dim*2 if self.kmeans else self.from_dim
                                              ,dim)
        ...
        if self.kmeans:
            ...
            if self.parametric:
                ...
                # self.queries2centroids = FullyConnectedLayer(dim, self.centroid_dim*self.num_heads)
                self.queries2centroids = torch.nn.Parameter(torch.randn([1, num_heads, to_len, self.centroid_dim]))
    ...
    def compute_centroids(self, _queries, queries, to_from, hw_shape):
        ...
        if to_from is None or self.parametric:
            if self.parametric:
                ...
                to_centroids = self.queries2centroids.tile([from_elements.shape[0], 1, 1, 1])
    ...
    def forward(self, from_tensor, to_tensor, from_pos, to_pos, 
            att_vars = None, att_mask = None, hw_shape = None, label=None,x_cat=None):
...
#
#     def __init__(self,in_channels, out_channels, kernel_size, bias=None, up=None,resample_kernel=None, gain=None):
#         super().__init__()
#         print('spade layer constructured')
#         pw = kernel_size // 2
#         if out_channels == 3:
...
class skip_op(torch.nn.Module):
    ...
    def __init__(self,in_channels, out_channels, kernel_size, bias, up,resample_kernel, gain,spade=False):
        ...
        self.spade = spade
        if self.spade == True:
            self.spade_norm = SPADE_norm_denorm(in_channels=in_channels,out_channels=out_channels,kernel_size=3)
...
class DecodingLayer(torch.nn.Module):
    ...
    def forward(self,input_feature_layer, x, y, att_vars = None, pos = None, mask = None,
                ...
                label=None,feature_from_encoder=None,**_kwargs):
...
class DecoderBlock_1spade(torch.nn.Module):
    ...
    def forward(self,input_feature_block,emb,label, x, img, ws, att_vars, fused_modconv = None, **layer_kwargs):
        ...
        layer_kwargs['feature_from_encoder'] = input_feature_block[-1]
...
class BipartiteDecoder_cat_nospade(torch.nn.Module):
    def __init__(self,
