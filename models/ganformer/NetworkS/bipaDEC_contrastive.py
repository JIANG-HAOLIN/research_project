...
import torch
import math
import models.ganformer.training_misc as misc
import models.ganformer.util_misc as torch_misc
from models.ganformer import persistence
from models.ganformer.ops import conv2d_resample,upfirdn2d,bias_act,fma
...
def normalize(x, eps = 1e-8, mode = "l2"):
    ...
    x = x.to(float_dtype())
    if mode == "l2":
        factor = x.square().mean(dim = dims, keepdim = True)
        factor = factor + eps
        factor = factor.rsqrt()
    if mode == "inf":
...
def get_res2channels(channel_base, channel_max):
    return lambda res: int(16384/res)


def encoder_get_res2channels(channel_base, channel_max):
    return lambda res: int(8192/res) if res != 256 else 35
...
class ResnetLayer(torch.nn.Module):
    ...
    def forward(self, x, _x):## _x is the output of self attention layer!!
        shape = x.shape
        batch = len(shape) > 2
...
@persistence.persistent_class
class ResnetLayer_skip(torch.nn.Module):
    def __init__(self, inchannels,outchannels, act = "linear", lrmul = 1, sa = False):
        super().__init__()
        self.fc0 = FullyConnectedLayer(inchannels, outchannels, act = act, lrmul = lrmul)
        self.fc1 = FullyConnectedLayer(outchannels, outchannels, lrmul = lrmul)
        self.skip = FullyConnectedLayer(inchannels, outchannels, lrmul = lrmul)
...
@persistence.persistent_class
class MLP(torch.nn.Module):##seems only used in Mapping network?
        ...
        for idx in range(self.layers_num):
            ...
            if resnet:
                if in_dim == out_dim:
                    ...
                    print('careful : indim != outdim !!')
                    layer = ResnetLayer_skip(in_dim,out_dim,act = act, lrmul = lrmul)
            else:
...
def get_sinusoidal_encoding(size, dim, num = 2):
    ...
    if num == 2:
        cy = torch.linspace(-1.0, 1.0, size).unsqueeze(-1)
        cx = torch.linspace(-1.0, 1.0, 2*size).unsqueeze(-1)
        i = torch.arange(int(dim / 4)).to(float_dtype())

        peSinx = torch.sin(cx / (torch.pow(10000.0, 4 * i / dim)))
        peCosx = torch.cos(cx / (torch.pow(10000.0, 4 * i / dim)))
        peSiny = torch.sin(cy / (torch.pow(10000.0, 4 * i / dim)))
        peCosy = torch.cos(cy / (torch.pow(10000.0, 4 * i / dim)))


        peSinX = peSinx.unsqueeze(0).tile([size, 1, 1])## size row, 1 column,
        peCosX = peCosx.unsqueeze(0).tile([size, 1, 1])
        peSinY = peSiny.unsqueeze(1).tile([1, 2*size, 1])## 1
        peCosY = peCosy.unsqueeze(1).tile([1, 2*size, 1])
...
class TransformerLayer(torch.nn.Module):
    ...
    def compute_centroids(self, _queries, queries, to_from, hw_shape):
        ...
        if to_from is not None:
            ...
            elif to_from.shape[-2] > self.to_len:
                print('need to downsample?')
            ...
            elif to_from.shape[-1] > self.from_len:
                ## can't upsample from_element here,or dimension won't match later
                to_from = upfirdn2d.downsample2d(to_from.reshape(-1, self.to_len, *hw_shape),
                                                f = nearest_neighbors_kernel(queries.device))
                to_from = to_from.reshape(-1, self.num_heads, self.to_len, self.from_len)
    ...
    def forward(self, from_tensor, to_tensor, from_pos, to_pos, 
        ...
        ### the 2d tensors will be reshaped to 4d tensors later in transpose_for_scores......
        att_vars = att_vars or {}
        to_from = att_vars.get("centroid_assignments")

        # Compute queries, keys and values
        queries = self.to_queries(from_tensor) ## Q = q(x)
        keys    = self.to_keys(to_tensor) ## K = k(y)
        values  = self.to_values(to_tensor) ## V = v(y)
        _queries = queries
...
class SynthesisLayer(torch.nn.Module):
    def __init__(self,
        ...
        **transformer_kwargs            # Arguments for SynthesisLayer
        ...
        if transformer:
            ...
            kwargs = {
                "from_len":  self.out_res * 2*self.out_res, "to_len":  k - 1,         # The from/to tensor lengths
    ...
    def forward(self, x, y, att_vars = None, pos = None, mask = None, noise_mode = "random", fused_modconv = True,**_kwargs):
        ...
        if self.local_noise and noise_mode != "none":
            if noise_mode == "random":
                noise = torch.randn([x.shape[0], 1, self.out_res, 2*self.out_res], device = x.device)
            if noise_mode == "const":
...
class SynthesisBlock(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs                      # Arguments for SynthesisLayer
        ...
        self.init_shape = (16,32)

        self.layer_num = 4 if resolution < 128 else 3

        for i in range(self.layer_num):
            synthesis_layer = SynthesisLayer(out_channels, out_channels, out_resolution = self.res,
            gain = 1 if self.stem else get_gain(architecture), 
            y_dim = w_dim, style = style, **layer_kwargs)
            setattr(self,f'conv{i+1}',synthesis_layer)
            self.num_conv += 1
    ...
    def forward(self, input_features, img, ws, att_vars, fused_modconv = None, **layer_kwargs):
        ...
        for i in range(self.layer_num):
            ...
            synthesis_layer = getattr(self, f'conv{i+1}')
            x, att_maps[0], att_vars = synthesis_layer(input_feature, next(w_iter), att_vars, fused_modconv = fused_modconv, **layer_kwargs)
        ...
        return x, img, att_maps, att_vars,output_features
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs                 # Arguments for SynthesisBlock
        ...
        self.img_channels = img_channels
        self.block_resolutions = [16,32,64,128,256]
        channels_num = get_res2channels(channel_base, channel_max) ##return a lambda function!!!!
        self.num_heads = block_kwargs.get("num_heads", 1) ##### return 1 if "num_heads" doesn't exist in **block_kwargs!!!!

        self.num_ws = 0
        for res in self.block_resolutions:
            in_channels = channels_num(res) if res > 4 else 0
            ...
            block = SynthesisBlock(in_channels, out_channels, w_dim = w_dim, k = k, resolution = res,
                img_channels = img_channels, is_last = False,
                transformer = True, **block_kwargs)
    ...
    def list2tensor(self, att_list, device):
        ...
        for att_map in att_list:
            ...
            att_map = att_map.reshape(-1, self.num_heads, self.k - 1, self.img_res, 2*self.img_res) # [NhkHW]
    ...
    def forward(self, ws, **block_kwargs):
        ...
        with torch.autograd.profiler.record_function("split_ws"):
            ...
            for res in self.block_resolutions:
                ...
                block_ws.append(ws.narrow(2, w_idx, block.num_conv))
        ...
        for res, cur_ws in (zip(self.block_resolutions, block_ws)):
            block = getattr(self, f"b{res}")
            x = block_kwargs['encoder_outputs'][idx]
            x, img, _att_maps, att_vars,output_features = block(x, img, cur_ws, att_vars, **block_kwargs)
            att_maps += _att_maps
            input_for_decoder.append(output_features)
            idx += 1
        # att_maps = self.list2tensor(att_maps, ws.device)

        # return img, att_maps
        return input_for_decoder
@persistence.persistent_class
class bipartite_attention_computer(torch.nn.Module):
    def __init__(self,
        ...
        **_kwargs                   # Ignore unrecognized keyword args
        ...
        ##random initialization of a vector with shape [ k-1 , w_dim ]
    ...
    def forward(self, z = None, c = None, ws = None, truncation_psi = 1, truncation_cutoff = None, return_img = True, 
        ...
        input_for_decoder = self.synthesis(ws, pos = self.pos, mask = mask, **synthesis_kwargs)
...
class EncodingLayer(torch.nn.Module):
    def __init__(self,
        ...
        **transformer_kwargs            # Arguments for SynthesisLayer
        ...
        self.local_noise = local_noise
        if local_noise:
            noise_shape = list(misc.crop_tensor_shape((self.out_res, 2*self.out_res),
                transformer_kwargs.get("crop_ratio")))
            self.register_buffer("noise_const", torch.randn(noise_shape))
            self.noise_strength = torch.nn.Parameter(torch.zeros([]))
    ...
    def forward(self, x, y, att_vars = None, pos = None, mask = None, noise_mode = "random", fused_modconv = True,**_kwargs):
        ...
        x = modulated_conv2d(x = x, weight = self.weight * self.w_gain, styles = self.affine(get_global(y)),
            modulate = self.style, up = self.up,down = self.down, padding = get_padding(self.kernel_size, "SAME"),
            resample_kernel = self.resample_kernel, flip_weight = (self.up == 1), fused_modconv = fused_modconv)
...
# A Encoder block that composes together multiple encoding layers (through standard, resnet or skip connections)
# Includes a stem initialization
@persistence.persistent_class
class EncoderBlock(torch.nn.Module):
    def __init__(self,
        ...
        out_channels,                       # Number of output channels
        w_dim,                              # Intermediate latent (W) dimensionality
        out_resolution,                         # Resolution of this block
        ...
        **layer_kwargs                      # Arguments for SynthesisLayer
        ...
        self.init_shape = (256,512)

        if self.stem:
            self.in_res = self.out_res
            self.conv0 = EncodingLayer(in_channels, out_channels,
                                       out_resolution=self.out_res, in_resolution=self.in_res,
                                       resample_kernel=resample_kernel,
                                       y_dim=w_dim, style=style, **layer_kwargs)

            self.conv1 = EncodingLayer(out_channels, out_channels,
            ...
            self.conv0 = EncodingLayer(in_channels, out_channels,
                                       out_resolution=self.out_res,in_resolution=self.in_res,down = 2,
                                       resample_kernel=resample_kernel,
                                        y_dim=w_dim, style=style, **layer_kwargs)
    ...
    def forward(self, x, img, ws, att_vars, fused_modconv = None, **layer_kwargs):
        ...
        x, att_maps[0], att_vars = self.conv0(x, next(w_iter), att_vars,
                                              fused_modconv = fused_modconv, **layer_kwargs)
        output_features.append(x)
        x, att_maps[0], att_vars = self.conv1(x, next(w_iter), att_vars,
                                              fused_modconv = fused_modconv, **layer_kwargs)
            ####此处用[0]是为了确保输出list可以后期拼接？？
        x = y.add_(x)
        output_features.append(x)
...
class EncoderNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs                 # Arguments for SynthesisBlock
        ...
        self.img_res = img_resolution
        self.img_channels = img_channels
        self.block_resolutions = [ 256 , 128 ,  64 , 32  , 16  ]## here is the output resolutiohn
        self.channel_nums =      [ 67  , 64  , 128 , 256 , 512 , 1024 , ]## input channel
        channels_num = encoder_get_res2channels(channel_base, channel_max) ##return a lambda function!!!!
        self.num_heads = block_kwargs.get("num_heads", 1) ##### return 1 if "num_heads" doesn't exist in **block_kwargs!!!!

        self.num_ws = 0
        for i,res in enumerate(self.block_resolutions):
            in_channels = self.channel_nums[i]
            out_channels = self.channel_nums[i+1]
            # is_last = (res == self.img_res)
            # use_transformer = transformer and np.log2(res) >= start_res and np.log2(res) < end_res
            block = EncoderBlock(in_channels, out_channels, w_dim = w_dim, k = k,
                                 out_resolution = res,
                                img_channels = img_channels, is_last = False,
                                transformer = True, **block_kwargs)
            self.num_ws += block.num_conv
            ...
            #     self.num_ws += block.num_torgb
            setattr(self, f"b{res}", block)
        print('EncoderNetwork Constructed')
    ...
    def forward(self,label, ws, **block_kwargs):
        block_ws = []
        with torch.autograd.profiler.record_function("split_ws"):
            torch_misc.assert_shape(ws, [None, self.k, self.num_ws, self.w_dim])
            ws = ws.to(torch.float32)
...
class BipartiteEncoder(torch.nn.Module):
    def __init__(self,
        ...
        self.encoder = EncoderNetwork(w_dim=w_dim, k=k, img_resolution=img_resolution,
                                          img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.encoder.num_ws

        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, k=k,
                                      num_broadcast=self.num_ws, **mapping_kwargs)

    def forward(self,label = None,z=None, c=None, ws=None, truncation_psi=1, truncation_cutoff=None, return_img=True,
                return_att=False, return_ws=False, subnet=None, **synthesis_kwargs):
        return_tensor = False
        ...
        input_for_decoder = self.encoder(label,ws, pos=self.pos, mask=mask, **synthesis_kwargs)
...
# A synthesis layer where intermediate latents (y) modulate the evolving image features (x)
@persistence.persistent_class
class ResConvEncodLayer(torch.nn.Module):
    def __init__(self,
        in_channels,                    # Number of input channels
        out_channels,                   # Number of output channels
        y_dim,                          # modulator dimensionality (W intermediate latents)
...
# Includes a stem initialization
@persistence.persistent_class
class ResConvEncodBlock(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block
        out_channels,                       # Number of output channels
        w_dim,                              # Intermediate latent (W) dimensionality
        ...
        **layer_kwargs                      # Arguments for SynthesisLayer
        ...
        if self.stem:
            self.in_res = self.out_res
            self.conv0 = ResConvEncodLayer(in_channels, out_channels,
                                       out_resolution=self.out_res, in_resolution=self.in_res,
                                       resample_kernel=resample_kernel,
                                       y_dim=w_dim, style=style, **layer_kwargs)

            self.conv1 = ResConvEncodLayer(out_channels, out_channels,
                                       out_resolution=self.out_res, in_resolution=self.out_res,
                                       gain=1 if self.stem else get_gain(architecture),
                                       y_dim=w_dim, style=style, **layer_kwargs)
            self.num_conv += 2
...
# - StyleGAN (--style)
@persistence.persistent_class
class ResConvEncodNetwork(torch.nn.Module):
    def __init__(self,
        w_dim,                         # Intermediate latent (W) dimensionality
        k,                             # Number of latent vector components w_1,...,w_k
        img_resolution,                # Output image resolution
        ...
        **block_kwargs                 # Arguments for SynthesisBlock
        ...
        for i,res in enumerate(self.block_resolutions):
            ...
            # is_last = (res == self.img_res)
            # use_transformer = transformer and np.log2(res) >= start_res and np.log2(res) < end_res
            block = ResConvEncodBlock(in_channels, out_channels, w_dim = w_dim, k = k,
                                 out_resolution = res,
                                img_channels = img_channels, is_last = False,
                                transformer = True, **block_kwargs)
            self.num_ws += block.num_conv
...
class ResConvEncoder(torch.nn.Module):
    def __init__(self,
                 z_dim,  # Input latent (Z) dimensionality
                 c_dim,  # Conditioning label (C) dimensionality
                 w_dim,  # Intermediate latent (W) dimensionality
        ...
        self.encoder = ResConvEncodNetwork(w_dim=w_dim, k=k, img_resolution=img_resolution,
                                          img_channels=img_channels, **synthesis_kwargs)
...
################################################################################################################
################################################################################################################
class Conv2dLayer_SPADE(torch.nn.Module):

    def __init__(self,in_channels, out_channels, kernel_size, bias, up,resample_kernel, gain):
        super().__init__()
        self.skip = Conv2dLayer(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_size,
                                bias=bias, up=up,resample_kernel=resample_kernel, gain=gain)
        if out_channels == 3:
            self.param_free_norm = torch.nn.InstanceNorm2d(out_channels, affine=False)  ##jhl
            self.spade_weight_modulation = EqualLinear(in_channels,out_channels, bias_init=1,activation='fused_lrelu')  # jhl
            self.spade_bias_modulation = EqualLinear(in_channels,out_channels, bias_init=1,activation='fused_lrelu')  # jhl
        else:
            self.param_free_norm = torch.nn.InstanceNorm2d(out_channels, affine=False)  ##jhl
            self.spade_weight_modulation = EqualLinear(out_channels,out_channels, bias_init=1,activation='fused_lrelu')  # jhl
            self.spade_bias_modulation = EqualLinear(out_channels,out_channels, bias_init=1,activation='fused_lrelu')  # jhl

    def forward(self,x,feature_from_encoder):
        x = self.skip(x)
        x = self.param_free_norm(x)
        spade_weight = self.spade_weight_modulation(feature_from_encoder.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        spade_bias = self.spade_bias_modulation(feature_from_encoder.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        x = x * spade_weight + spade_bias
...
class EqualLinear(torch.nn.Module):
    ...
    def forward(self, input):
        if self.activation:
            out = torch.nn.functional.linear(input, self.weight * self.scale) + self.bias
...
@persistence.persistent_class
class DecoderToRGBLayer(torch.nn.Module):
    def __init__(self, in_channels, out_channels, y_dim, kernel_size = 1, style = True):
        ...
        self.param_free_norm = torch.nn.InstanceNorm2d(out_channels, affine=False)  ##jhl
        self.spade_weight_modulation = EqualLinear(in_channels, out_channels, bias_init=1,
                                                   activation='fused_lrelu')  # jhl
        self.spade_bias_modulation = EqualLinear(in_channels, out_channels, bias_init=1,
                                                 activation='fused_lrelu')  # jhl


    def forward(self,feature_from_encoder, x, y, fused_modconv):
...
# A synthesis layer where intermediate latents (y) modulate the evolving image features (x)
@persistence.persistent_class
class DecodingLayer(torch.nn.Module):
    def __init__(self,
        in_channels,                    # Number of input channels
        out_channels,                   # Number of output channels
        y_dim,                          # modulator dimensionality (W intermediate latents)
        ...
        gain            = 1,            # Scaling factor for the output tensor
        style           = True,         # Use modulated convolution
        transformer     = True,        # Whereas the generator uses attention or not (i.e. GANformer or StyleGAN)
        use_pos         = False,        # Use positional encoding for latents
        ltnt_gate       = False,        # Gate attention from latents, such that components may not send information
                                        # when gate value is low
        img_gate        = False,        # Gate attention for images, such that some image positions may not get updated
        ...
        **transformer_kwargs            # Arguments for SynthesisLayer
        ...
        if self.out_channels == 3:
            self.param_free_norm = torch.nn.InstanceNorm2d(self.out_channels, affine=False)  ##jhl
            self.spade_weight_modulation = EqualLinear(self.in_channels, self.out_channels, bias_init=1,activation='fused_lrelu')  # jhl
            self.spade_bias_modulation = EqualLinear(self.in_channels, self.out_channels, bias_init=1,activation='fused_lrelu')  # jhl
        else:
            self.param_free_norm = torch.nn.InstanceNorm2d(self.out_channels, affine=False)  ##jhl
            self.spade_weight_modulation = EqualLinear(self.out_channels, self.out_channels, bias_init=1,activation='fused_lrelu')  # jhl
            self.spade_bias_modulation = EqualLinear(self.out_channels, self.out_channels, bias_init=1,activation='fused_lrelu')  # jhl
    ...
    def forward(self,input_feature_layer, x, y, att_vars = None, pos = None, mask = None, noise_mode = "random", fused_modconv = True,**_kwargs):
        assert noise_mode in ["random", "const", "none"]
        torch_misc.assert_shape(x, [None, self.weight.shape[1], self.in_res, 2*self.in_res])
        ### y is latent [ B ,17, 32]
        att_map, noise = None, None
        ...
        ##### 'SPADE_like' modulation ############
        x = self.param_free_norm(x)
        spade_weight = self.spade_weight_modulation(input_feature_layer.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        spade_bias = self.spade_bias_modulation(input_feature_layer.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        x = x * spade_weight + spade_bias
...
# A Decoder block that composes together multiple decoding layers (through standard, resnet or skip connections)
# Includes a stem initialization
@persistence.persistent_class
class DecoderBlock(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs                      # Arguments for SynthesisLayer
        ...
        if self.stem:
            self.in_res = self.out_res
            self.conv0 = DecodingLayer(in_channels, out_channels,
            ...
            if self.architecture == 'resnet':
                self.skip = Conv2dLayer_SPADE(in_channels, out_channels, kernel_size=1, bias=False, up = 1,
                                        resample_kernel=resample_kernel, gain=get_gain(architecture))
        elif self.is_last:
            self.in_res = self.out_res//2
            self.conv0 = DecodingLayer(in_channels, out_channels,
            ...
            if self.architecture == 'resnet':
                self.skip = Conv2dLayer_SPADE(in_channels, out_channels, kernel_size=1, bias=False, up = 2,
                                        resample_kernel=resample_kernel, gain=get_gain(architecture))
        ...
        if is_last or architecture == "skip":
            self.torgb = DecoderToRGBLayer(out_channels, img_channels, y_dim = w_dim, style = style)
            self.num_torgb += 1
        ...
        if is_last: # For backward compatibility with TF version
            ...
            for disabled in ["transformer", "bias", "local_noise"]:
                conv_last_kwargs[disabled] = False
            self.conv_last = DecodingLayer(in_channels, out_channels,
                                           out_resolution = self.out_res,in_resolution=self.out_res,
                                            y_dim = w_dim, style = style,
                                           **conv_last_kwargs)
            self.num_conv += 1
    ...
    def forward(self,input_feature_block,emb, x, img, ws, att_vars, fused_modconv = None, **layer_kwargs):
        ...
        if self.stem:
            ...
            att_maps = [None, None]
            output_features = []
            y = self.skip(x, input_feature_block[-1])

            x, att_maps[0], att_vars = self.conv0(input_feature_block[-2], x, next(w_iter), att_vars,
                                                  fused_modconv=fused_modconv, **layer_kwargs)
            output_features.append(x)
            x = torch.cat([x, emb[f'res{self.out_res}'][0]],dim=1)
            x, att_maps[0], att_vars = self.conv1(input_feature_block[-3], x, next(w_iter), att_vars,
                                                  fused_modconv=fused_modconv, **layer_kwargs)
            ####此处用[0]是为了确保输出list可以后期拼接？？
            x = y.add_(x)
            output_features.append(x)
            ...
            torch_misc.assert_shape(x, [None, self.in_channels, self.in_res,2*self.in_res])
            # Main layers
            att_maps = [None, None]
            output_features = []
            y = self.skip(x,input_feature_block[-1])
        ...
        # ToRGB
        if img is not None:
            torch_misc.assert_shape(img, [None, self.img_channels, self.in_res, 2*self.in_res])
            img = upfirdn2d.upsample2d(img, self.resample_kernel)
        if self.is_last: # For backward compatibility with TF version
            x = self.conv_last(input_feature_block[-4],x, next(w_iter), fused_modconv = fused_modconv,
                               **layer_kwargs)[0]
        if self.is_last or self.architecture == "skip":
            y = self.torgb(input_feature_block[-5],x, next(w_iter), fused_modconv = fused_modconv)
            img = img.add_(y) if img is not None else y
####################
...
class DecoderNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs                 # Arguments for SynthesisBlock
        ...
        self.img_res = img_resolution
        self.img_channels = img_channels
        self.block_resolutions = [ 16   , 32  ,  64 , 128 , 256  ]## output resolution
        self.channel_nums =      [ 1024 , 512 , 256 , 128 , 64 , 32 ]## input channel
        # channels_num = encoder_get_res2channels(channel_base, channel_max) ##return a lambda function!!!!
        self.num_heads = block_kwargs.get("num_heads", 1) ##### return 1 if "num_heads" doesn't exist in **block_kwargs!!!!
        ...
        for i,res in enumerate(self.block_resolutions):
            ...
            # use_transformer = transformer and np.log2(res) >= start_res and np.log2(res) < end_res
            stem = (i == 0)
            is_last = (i == len(self.block_resolutions) - 1)
            block = DecoderBlock(in_channels, out_channels, w_dim = w_dim, k = k,
                                 out_resolution = res,
                                img_channels = img_channels, is_last = is_last,
                                transformer = True,stem =stem, **block_kwargs)
    ...
    def forward(self,from_encoder,emb,label, ws, **block_kwargs):
        block_ws = []
        with torch.autograd.profiler.record_function("split_ws"):
            torch_misc.assert_shape(ws, [None, self.k, self.num_ws, self.w_dim])
            ws = ws.to(torch.float32)
...
class BipartiteDecoder_contrastive(torch.nn.Module):
    def __init__(self,
                 z_dim,  # Input latent (Z) dimensionality
                 c_dim,  # Conditioning label (C) dimensionality
                 w_dim,  # Intermediate latent (W) dimensionality
                 ...
                 component_dropout=0.0,  # Dropout over the latent components, 0 = disable
                 mapping_kwargs={},  # Arguments for MappingNetwork
                 decoder_kwargs={},  # Arguments for SynthesisNetwork
                 **_kwargs  # Ignore unrecognized keyword args
                 ):
        super().__init__()
        ...
        self.decoder = DecoderNetwork(w_dim=w_dim, k=k, img_resolution=img_resolution,
                                          img_channels=img_channels, **decoder_kwargs)
        self.num_ws = self.decoder.num_ws

        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, k=k,
                                      num_broadcast=self.num_ws, **mapping_kwargs)

    def forward(self,from_encoder=None,emb=None,label = None,z=None, c=None, ws=None, truncation_psi=1, truncation_cutoff=None, return_img=True,
                return_att=False, return_ws=False, subnet=None, **synthesis_kwargs):
        return_tensor = False
...
class MappingNetwork(torch.nn.Module):
    def __init__(self,
                 ...
                 c_dim=35,  # Conditioning label (C) dimensionality, 0 = no label
