...
class DecoderBlock_1spade(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs                      # Arguments for SynthesisLayer
        ...
        if self.stem:
            ...
            self.project_shared = torch.nn.Sequential(
                                                torch.nn.Conv2d(35+out_channels, 128, kernel_size=3, padding=1),
                                                torch.nn.ReLU()
                                            )
            ...
            self.project0 = torch.nn.Conv2d(128,out_channels,kernel_size=3,padding=1)
            ...
            self.conv1 = DecodingLayer(in_channels, out_channels,
                                       ...
                                       y_dim=w_dim, style=style, **layer_kwargs)
            self.project1 = torch.nn.Conv2d(128,out_channels, kernel_size=3, padding=1)
            self.num_conv += 2
            ...
            self.spade0 = SPADE_norm_denorm(in_channels=128,out_channels=out_channels)
    ...
    def forward(self,input_feature_block,emb,label, x, img, ws, att_vars, fused_modconv = None, **layer_kwargs):
        ...
        if self.stem:
            label = torch.cat([label,emb[f'res{self.out_res}'][0]],dim = 1)
            project_act = self.project_shared(label)
...
class DecoderNetwork_1spade(torch.nn.Module):
    ...
    def forward(self,from_encoder,emb,label, ws, **block_kwargs):
        ...
        for res, cur_ws in (zip(self.block_resolutions, block_ws)):
            ...
            block = getattr(self, f"b{res}")
            block_label = torch.nn.functional.interpolate(label,size=[res,2*res],mode='nearest')
            x, img, _att_maps, att_vars,output_features = block(None,emb,block_label,x, img, cur_ws, att_vars, **block_kwargs)
            att_maps += _att_maps
...
class BipartiteDecoder_1spade_noENC_catFeature(torch.nn.Module):
    def __init__(self,
