...
from models.blocks import Conv_encoder_avepool_for_bipartite_decoder
...
class TransformerLayer(torch.nn.Module):
    def __init__(self,
        ...
        if is_D:
            self.to_queries = FullyConnectedLayer(32 + from_dim, dim)
        # self.to_queries = FullyConnectedLayer(from_dim,dim)
        ...
        self.from_gate_attention = GateAttention(from_gate, dim, pos_dim, num_heads, from_len=from_len, to_len=1,
                                                 gate_bias=1)
    ...
    def compute_centroids(self, _queries, queries, to_from, hw_shape):
        ...
        from_elements = transpose_for_scores(from_elements, self.num_heads, self.from_len,
                                             self.centroid_dim)  # [B, N, F, H]
    ...
    def __init__(self,
        ...
        **transformer_kwargs            # Arguments for SynthesisLayer
        ...
        if transformer:
            ...
            self.transformer = TransformerLayer(dim = out_channels, is_D = is_D, **kwargs)
...
class DecoderBlock_1spade(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs                      # Arguments for SynthesisLayer
        ...
        if self.stem:
            ...
            ####
            self.const = torch.nn.Parameter(torch.randn([in_channels // 2, self.in_res, self.in_res*2]))
            ####
            self.conv0 = DecodingLayer(in_channels, out_channels,
                                       out_resolution=self.out_res, in_resolution=self.in_res,up = 1,
                                       resample_kernel=resample_kernel,
                                       y_dim=w_dim, style=style,is_D=True,
                                       **layer_kwargs)
    ...
    def forward(self,input_feature_block,emb,label, x, img, ws, att_vars, fused_modconv = None, **layer_kwargs):
        ...
        if self.stem:
            ...
            # x = self.const.unsqueeze(0).repeat([batch_size, 1, 1, 1]).to(ws.device)
            # print("ganformer original initial method")
...
# - StyleGAN (--style)
@persistence.persistent_class
class DecoderNetwork_clade(torch.nn.Module):
    def __init__(self,
        w_dim,                         # Intermediate latent (W) dimensionality
        k,                             # Number of latent vector components w_1,...,w_k
...
class clade_simplex(torch.nn.Module):
    def __init__(self,
        ...
        self.decoder = DecoderNetwork_clade(w_dim=w_dim, k=k, img_resolution=img_resolution,
    ...
    def forward(self,encoder_outputs=None,emb=None,label = None,z=None, c=None, ws=None, truncation_psi=1, truncation_cutoff=None, return_img=True,
        ...
        assert label.shape[1] == (self.k - 1)

        clade_input = torch.einsum('bchw,bcs->bshw',label,ws[:,:-1,0,:])
        # clade_input = torch.cat([label_with_style,emb['res256']],dim = 1)


        output_of_decoder,img = self.decoder(encoder_outputs,emb,clade_input,ws, pos=self.pos, mask=mask, **synthesis_kwargs)
