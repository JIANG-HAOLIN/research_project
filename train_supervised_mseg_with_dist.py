...
#--- create models ---#
opt.seg_path = os.path.abspath(os.path.join(__file__,"..","pretrained_models","16_best_miou_0.3742395.pth"))
# model = models_EPE.EPE_model_unet_ft(opt)
model = models_EPE.EPE_model_mseg_ft(opt)
...
optimizerS = torch.optim.Adam(model.module.seg_net_pretrained.model.parameters(), lr=0.0001   , betas=(opt.beta1, opt.beta2))
...
for epoch in range(start_epoch, opt.num_epochs):
    ...
    for i, data_i in enumerate(dataloader_supervised):
        ...
        image, label, label_map, instance_map, dist_map = models_EPE.preprocess_input_with_dist(opt, data_i)
