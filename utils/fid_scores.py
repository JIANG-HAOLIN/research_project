...
class fid_pytorch():
    ...
    def compute_fid_with_valid_path(self, netG, netEMA,model = None):
        ...
        with torch.no_grad():
            for i, data_i in enumerate(self.val_dataloader):
                ...
                dist_map = distance_map(label_map.squeeze(1).to('cpu'), norm='norm').to(fid_scores_device)
                ...
                noise = mixing_noise(self.opt.batch_size, 512, 0, fid_scores_device)
    ...
    def update_logs(self, cur_fid, epoch):
        ...
        plt.title(f'best fid:{self.best_fid}')
