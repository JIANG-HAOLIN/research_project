...
class losses_saver():
    def __init__(self, opt):
        if opt.model_supervision == 2:
            self.name_list = ["Generator", "Vgg", "D_fake", "D_real", "LabelMix","Realism"]
    ...
    if condition_run_stats:
        with torch.no_grad():
            ...
            for i, data_i in enumerate(dataloader):
                ...
                dist_map = distance_map(label_map.squeeze(1).to('cpu'), norm='norm').to(utils_device)
                ...
                noise = mixing_noise(opt.batch_size, 512, 0, utils_device)
...
class image_saver():
    ...
    def visualize_batch(self,
        ...
        with torch.no_grad():
            ...
            fake,_ = model.module.netG(       label=label,
                                             label_class_dict=label_class_dict,
                                             coords=converted,
                                             latent=latent,
                                             return_latents=False,
...
class image_saver_all_in_one():
    ...
    def visualize_batch(self,
        ...
        with torch.no_grad():
            ...
            if not self.opt.no_EMA:
                model.eval()
                fake_ema,_ = model.module.netEMA(label=label,
                                                 label_class_dict=label_class_dict,
                                                 coords=converted,
                                                 latent=latent,
                                                 return_latents=False,
